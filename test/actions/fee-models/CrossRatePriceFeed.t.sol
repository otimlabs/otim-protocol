...
contract CrossRatePriceFeedTest is Test {
    ...
    function test_latestRoundData() public view {
        ...
        (uint80 numeratorRoundId, int256 numeratorAnswer,, uint256 numeratorUpdatedAt, uint80 numeratorAnsweredInRound)
        = usdcUsdFeed.latestRoundData();
        (, int256 denominatorAnswer,, uint256 denominatorUpdatedAt,) = ethUsdFeed.latestRoundData();

        // Get cross-rate data
        (uint80 crossRoundId, int256 crossAnswer,, uint256 crossUpdatedAt, uint80 crossAnsweredInRound) =
        ...
        assertEq(
            crossAnsweredInRound, numeratorAnsweredInRound, "Cross rate should use numerator feed's answered in round"
        );
    }
    ...
    function test_latestRoundDataConsistency() public view {
        ...
        (uint80 roundId2, int256 answer2, uint256 startedAt2, uint256 updatedAt2, uint80 answeredInRound2) =
            crossRatePriceFeed.latestRoundData();
        (uint80 roundId3, int256 answer3, uint256 startedAt3, uint256 updatedAt3, uint80 answeredInRound3) =
            crossRatePriceFeed.latestRoundData();
