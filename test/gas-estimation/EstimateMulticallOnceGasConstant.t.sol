...
contract EstimateMulticallOnceGasConstant is InstructionForkTestContext {
    using InstructionLib for InstructionLib.Instruction;

    Treasury treasury;
    FeeTokenRegistry feeTokenRegistry;

    MulticallOnceAction multicallOnceAction;
    ...
    uint256 public constant MULTICALL_ONCE_ACTION_GAS_CONSTANT = 130_000;
    ...
    constructor() {
        ...
        multicallOnceAction = new MulticallOnceAction(
            address(instructionStorage),
            address(feeTokenRegistry),
            address(treasury),
            MULTICALL_ONCE_ACTION_GAS_CONSTANT
        );

        actionManager.addAction(address(multicallOnceAction));
    }

    // check that the CALL_ONCE_GAS_CONSTANT doesn't result in an underpayment of the fee
    function testFuzz_multicallOnce_gasConstant(uint256 salt, IMulticallOnceAction.MulticallOnce memory arguments)
        public
    {
        uint256 totalValue;
        // always allow failure so the fuzzer can't force a failure
        for (uint256 i = 0; i < arguments.subcalls.length; i++) {
            arguments.subcalls[i].allowFailure = true;

            // assume value is not ridiculously high
            vm.assume(arguments.subcalls[i].value < 100 ether);
            // assume gasLimit is not ridiculously high
            vm.assume(arguments.subcalls[i].gasLimit < 30_000_000);

            totalValue += arguments.subcalls[i].value;
        }
        ...
        vm.deal(address(user), totalValue);

        // build Instruction with fuzz values (maxExecutions is set to 1)
        buildInstruction(salt, 1, address(multicallOnceAction), abi.encode(arguments));
