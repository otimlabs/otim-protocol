...
contract EstimateConditionalSkipCCTPTransferGasConstant is InstructionForkTestContext {
    ...
    ConditionalSkipCCTPTransferAction conditionalSkipCCTPTransferAction;
    ...
    uint256 public constant CONDITIONAL_SKIP_CCTP_TRANSFER_ACTION_GAS_CONSTANT = 103_500;
    ...
    constructor() {
        ...
        conditionalSkipCCTPTransferAction = new ConditionalSkipCCTPTransferAction(
            ...
            address(treasury),
            CONDITIONAL_SKIP_CCTP_TRANSFER_ACTION_GAS_CONSTANT
        );
    ...
    function testFuzz_conditionalSkipCCTPTransfer_gasConstant(
        uint256 salt,
        uint256 maxExecutions,
        IConditionalSkipCCTPTransferAction.ConditionalSkipCCTPTransfer memory arguments
    ) public {
        arguments.destinationDomain = DESTINATION_DOMAIN;
        arguments.destinationMintRecipient = DESTINATION_MINT_RECIPIENT;

        vm.assume(
            arguments.threshold > 2000 && arguments.threshold < IERC20(SEPOLIA_USDC).balanceOf(SEPOLIA_USDC_WHALE)
        );
        ...
        buildInstruction(salt, maxExecutions, address(conditionalSkipCCTPTransferAction), abi.encode(arguments));
