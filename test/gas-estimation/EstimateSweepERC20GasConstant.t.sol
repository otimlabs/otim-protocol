...
contract EstimateSweepERC20GasConstant is InstructionForkTestContext {
    ...
    constructor() {
        ...
        sweepERC20Action =
            new SweepERC20Action(address(feeTokenRegistry), address(treasury), SWEEP_ERC20_ACTION_GAS_CONSTANT);

        actionManager.addAction(address(sweepERC20Action));
    }

    // check that the SWEEP_ERC20_ACTION_GAS_CONSTANT doesn't result in an underpayment of the fee
    function testFuzz_sweepERC20_gasConstant(
        uint256 salt,
        uint256 maxExecutions,
        ISweepERC20Action.SweepERC20 memory arguments
    ) public {
        arguments.token = SEPOLIA_USDC;
        // disregard fuzz generated target
        arguments.target = target.addr;
        ...
        buildInstruction(salt, maxExecutions, address(sweepERC20Action), abi.encode(arguments));
