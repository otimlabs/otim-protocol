...
contract EstimateSweepGasConstant is InstructionForkTestContext {
    ...
    uint256 public constant SWEEP_ACTION_GAS_CONSTANT = 104_000;
    ...
    constructor() {
        ...
        sweepAction = new SweepAction(address(feeTokenRegistry), address(treasury), SWEEP_ACTION_GAS_CONSTANT);

        actionManager.addAction(address(sweepAction));
    }

    // check that the SWEEP_ACTION_GAS_CONSTANT doesn't result in an underpayment of the fee
    function testFuzz_sweep_gasConstant(uint256 salt, uint256 maxExecutions, ISweepAction.Sweep memory arguments)
        public
    {
