...
contract DeploySweepERC20Action is Script {
    ...
    //
    // forge script DeploySweepERC20Action

    // commands to deterministically deploy (and check the expected address before deploying):
    //
    // - with private key (on Anvil): forge script DeploySweepERC20Action --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeploySweepERC20Action --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeploySweepERC20Action --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeploySweepERC20Action --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        ...
        uint256 gasConstant = vm.envUint("SWEEP_ERC20_ACTION_GAS_CONSTANT");
        ...
        // deterministically deploy SweepERC20Action via canonical Create2 deployer
        SweepERC20Action sweepERC20Action =
            new SweepERC20Action{salt: salt}(feeTokenRegistryAddress, treasuryAddress, gasConstant);

        vm.stopBroadcast();

        console2.log("SweepERC20Action deployed at:", address(sweepERC20Action));
    }
        ...
        view
    {
        ...
        address expectedAddress = vm.envAddress("EXPECTED_SWEEP_ERC20_ACTION_ADDRESS");
        ...
        address calculatedAddress = vm.computeCreate2Address(
            ...
            keccak256(
                abi.encodePacked(
                    type(SweepERC20Action).creationCode,
                    abi.encode(feeTokenRegistryAddress, treasuryAddress, gasConstant)
                )
            )
        );
