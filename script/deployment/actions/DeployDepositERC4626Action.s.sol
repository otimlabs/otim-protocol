...
contract DeployDepositERC4626Action is Script {
    ...
    //
    // forge script DeployDepositERC4626Action

    // commands to deterministically deploy (and check the expected address before deploying):
    //
    // - with private key (on Anvil): forge script DeployDepositERC4626Action --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeployDepositERC4626Action --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeployDepositERC4626Action --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeployDepositERC4626Action --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        ...
        uint256 gasConstant = vm.envUint("DEPOSIT_ERC4626_ACTION_GAS_CONSTANT");
        ...
        // deterministically deploy DepositERC4626Action via canonical Create2 deployer
        DepositERC4626Action depositERC4626Action =
            new DepositERC4626Action{salt: salt}(feeTokenRegistryAddress, treasuryAddress, gasConstant);

        vm.stopBroadcast();

        console2.log("DepositERC4626Action deployed at:", address(depositERC4626Action));
    }
    ...
    function checkExpectedAddress(address feeTokenRegistry, address treasuryAddress, uint256 gasConstant) public view {
        ...
        address expectedAddress = vm.envAddress("EXPECTED_DEPOSIT_ERC4626_ACTION_ADDRESS");
        ...
        address calculatedAddress = vm.computeCreate2Address(
            ...
            keccak256(
                abi.encodePacked(
                    type(DepositERC4626Action).creationCode, abi.encode(feeTokenRegistry, treasuryAddress, gasConstant)
                )
            )
        );
