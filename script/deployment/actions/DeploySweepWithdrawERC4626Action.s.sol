...
contract DeploySweepWithdrawERC4626Action is Script {
    ...
    //
    // forge script DeploySweepWithdrawERC4626Action

    // commands to deterministically deploy (and check the expected address before deploying):
    //
    // - with private key (on Anvil): forge script DeploySweepWithdrawERC4626Action --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeploySweepWithdrawERC4626Action --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeploySweepWithdrawERC4626Action --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeploySweepWithdrawERC4626Action --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        ...
        uint256 gasConstant = vm.envUint("SWEEP_WITHDRAW_ERC4626_ACTION_GAS_CONSTANT");
        ...
        console2.log("SweepWithdrawERC4626Action deployed at:", address(sweepWithdrawERC4626Action));
    }
    ...
    function checkExpectedAddress(address feeTokenRegistry, address treasuryAddress, uint256 gasConstant) public view {
        ...
        address expectedAddress = vm.envAddress("EXPECTED_SWEEP_WITHDRAW_ERC4626_ACTION_ADDRESS");
        ...
        address calculatedAddress = vm.computeCreate2Address(
            ...
            keccak256(
                abi.encodePacked(
                    type(SweepWithdrawERC4626Action).creationCode,
