...
contract DeployDeactivateInstructionAction is Script {
    ...
    //
    // forge script DeployDeactivateInstructionAction

    // commands to deterministically deploy (and check the expected address before deploying):
    //
    // - with private key (on Anvil): forge script DeployDeactivateInstructionAction --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeployDeactivateInstructionAction --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeployDeactivateInstructionAction --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeployDeactivateInstructionAction --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        ...
        if (vm.isContext(VmSafe.ForgeContext.ScriptBroadcast)) {
            checkExpectedAddress(instructionStorageAddress, feeTokenRegistryAddress, treasuryAddress, gasConstant);
        }

        vm.startBroadcast();

        // deterministically deploy DeactivateInstructionAction contracts via canonical Create2 deployer
        ...
        console2.log("DeactivateInstructionAction deployed at:", address(deactivateAction));
    }
    ...
    function checkExpectedAddress(
        ...
        address expectedAddress = vm.envAddress("EXPECTED_DEACTIVATE_INSTRUCTION_ACTION_ADDRESS");
        ...
        address calculatedAddress = vm.computeCreate2Address(
            ...
            keccak256(
                abi.encodePacked(
                    type(DeactivateInstructionAction).creationCode,
                    abi.encode(instructionStorageAddress, feeTokenRegistryAddress, treasuryAddress, gasConstant)
                )
            )
        );
