...
contract DeployTransferOnceAction is Script {
    ...
    //
    // - with private key (on Anvil): forge script DeployTransferOnceAction --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeployTransferOnceAction --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeployTransferOnceAction --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeployTransferOnceAction --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        ...
        uint256 gasConstant = vm.envUint("TRANSFER_ONCE_ACTION_GAS_CONSTANT");
        ...
        // deterministically deploy TransferOnceAction contracts via canonical Create2 deployer
        TransferOnceAction transferAction =
            new TransferOnceAction{salt: salt}(feeTokenRegistryAddress, treasuryAddress, gasConstant);

        vm.stopBroadcast();

        console2.log("TransferOnceAction deployed at:", address(transferAction));
    }
    ...
    function checkExpectedAddress(address feeTokenRegistry, address treasuryAddress, uint256 gasConstant) public view {
        ...
        address calculatedAddress = vm.computeCreate2Address(
            ...
            keccak256(
                abi.encodePacked(
                    type(TransferOnceAction).creationCode, abi.encode(feeTokenRegistry, treasuryAddress, gasConstant)
                )
            )
        );
