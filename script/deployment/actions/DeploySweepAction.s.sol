...
contract DeploySweepAction is Script {
    ...
    //
    // forge script DeploySweepAction

    // commands to deterministically deploy (and check the expected address before deploying):
    //
    // - with private key (on Anvil): forge script DeploySweepAction --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeploySweepAction --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeploySweepAction --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeploySweepAction --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        ...
        uint256 gasConstant = vm.envUint("SWEEP_ACTION_GAS_CONSTANT");
        ...
        // deterministically deploy SweepAction via canonical Create2 deployer
        SweepAction sweepAction = new SweepAction{salt: salt}(feeTokenRegistryAddress, treasuryAddress, gasConstant);
