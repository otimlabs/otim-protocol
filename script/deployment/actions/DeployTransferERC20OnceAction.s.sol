...
contract DeployTransferERC20OnceAction is Script {
    ...
    //
    // forge script DeployTransferERC20OnceAction

    // commands to deterministically deploy (and check the expected address before deploying):
    //
    // - with private key (on Anvil): forge script DeployTransferERC20OnceAction --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeployTransferERC20OnceAction --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeployTransferERC20OnceAction --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeployTransferERC20OnceAction --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        ...
        console2.log("TransferERC20OnceAction deployed at:", address(transferAction));
    }
    ...
    function checkExpectedAddress(address feeTokenRegistry, address treasuryAddress, uint256 gasConstant) public view {
        ...
        address calculatedAddress = vm.computeCreate2Address(
            ...
            keccak256(
                abi.encodePacked(
                    type(TransferERC20OnceAction).creationCode,
