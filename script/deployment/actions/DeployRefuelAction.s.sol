...
contract DeployRefuelAction is Script {
    ...
    //
    // forge script DeployRefuelAction

    // commands to deterministically deploy (and check the expected address before deploying):
    //
    // - with private key (on Anvil): forge script DeployRefuelAction --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeployRefuelAction --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeployRefuelAction --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeployRefuelAction --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        ...
        uint256 gasConstant = vm.envUint("REFUEL_ACTION_GAS_CONSTANT");
        ...
        // deterministically deploy RefuelAction via canonical Create2 deployer
        RefuelAction refuelAction = new RefuelAction{salt: salt}(feeTokenRegistryAddress, treasuryAddress, gasConstant);

        vm.stopBroadcast();

        console2.log("RefuelAction deployed at:", address(refuelAction));
    }
    ...
    function checkExpectedAddress(address feeTokenRegistry, address treasuryAddress, uint256 gasConstant) public view {
        ...
        address expectedAddress = vm.envAddress("EXPECTED_REFUEL_ACTION_ADDRESS");
        ...
        address calculatedAddress = vm.computeCreate2Address(
            ...
            keccak256(
                abi.encodePacked(
                    type(RefuelAction).creationCode, abi.encode(feeTokenRegistry, treasuryAddress, gasConstant)
                )
            )
        );
