...
contract DeploySweepDepositAccountERC20Action is Script {
    ...
    //
    // forge script DeploySweepDepositAccountERC20Action

    // commands to deterministically deploy (and check the expected address before deploying):
    //
    // - with private key (on Anvil): forge script DeploySweepDepositAccountERC20Action --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeploySweepDepositAccountERC20Action --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeploySweepDepositAccountERC20Action --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeploySweepDepositAccountERC20Action --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        ...
        uint256 gasConstant = vm.envUint("SWEEP_DEPOSIT_ACCOUNT_ERC20_ACTION_GAS_CONSTANT");
        ...
        // deterministically deploy SweepDepositAccountERC20Action contract via canonical Create2 deployer
        SweepDepositAccountERC20Action sweepDepositAccountERC20Action =
            new SweepDepositAccountERC20Action{salt: salt}(feeTokenRegistryAddress, treasuryAddress, gasConstant);

        vm.stopBroadcast();

        console2.log("SweepDepositAccountERC20Action deployed at:", address(sweepDepositAccountERC20Action));
    }
    ...
    function checkExpectedAddress(address feeTokenRegistry, address treasuryAddress, uint256 gasConstant) public view {
        ...
        address expectedAddress = vm.envAddress("EXPECTED_SWEEP_DEPOSIT_ACCOUNT_ERC20_ACTION_ADDRESS");
        ...
        address calculatedAddress = vm.computeCreate2Address(
            ...
            keccak256(
                abi.encodePacked(
                    type(SweepDepositAccountERC20Action).creationCode,
