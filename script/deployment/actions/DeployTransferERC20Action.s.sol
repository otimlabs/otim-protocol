...
contract DeployTransferERC20Action is Script {
    ...
    function run() public {
        ...
        TransferERC20Action transferAction =
            new TransferERC20Action{salt: salt}(feeTokenRegistryAddress, treasuryAddress, gasConstant);

        vm.stopBroadcast();

        console2.log("TransferERC20Action deployed at:", address(transferAction));
    }
    ...
    function checkExpectedAddress(address feeTokenRegistry, address treasuryAddress, uint256 gasConstant) public view {
        ...
        address calculatedAddress = vm.computeCreate2Address(
            ...
            keccak256(
                abi.encodePacked(
                    type(TransferERC20Action).creationCode, abi.encode(feeTokenRegistry, treasuryAddress, gasConstant)
                )
            )
        );
