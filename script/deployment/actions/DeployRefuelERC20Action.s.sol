...
contract DeployRefuelERC20Action is Script {
    ...
    //
    // forge script DeployRefuelERC20Action

    // commands to deterministically deploy (and check the expected address before deploying):
    //
    // - with private key (on Anvil): forge script DeployRefuelERC20Action --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeployRefuelERC20Action --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeployRefuelERC20Action --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeployRefuelERC20Action --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        ...
        uint256 gasConstant = vm.envUint("REFUEL_ERC20_ACTION_GAS_CONSTANT");
        ...
        // deterministically deploy RefuelERC20Action via canonical Create2 deployer
        RefuelERC20Action refuelERC20Action =
            new RefuelERC20Action{salt: salt}(feeTokenRegistryAddress, treasuryAddress, gasConstant);

        vm.stopBroadcast();

        console2.log("RefuelERC20Action deployed at:", address(refuelERC20Action));
    }
    ...
    function checkExpectedAddress(address feeTokenRegistry, address treasuryAddress, uint256 gasConstant) public view {
        ...
        address expectedAddress = vm.envAddress("EXPECTED_REFUEL_ERC20_ACTION_ADDRESS");
        ...
        address calculatedAddress = vm.computeCreate2Address(
            ...
            keccak256(
                abi.encodePacked(
                    type(RefuelERC20Action).creationCode, abi.encode(feeTokenRegistry, treasuryAddress, gasConstant)
                )
            )
        );
