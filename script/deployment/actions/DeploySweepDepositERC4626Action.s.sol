...
contract DeploySweepDepositERC4626Action is Script {
    ...
    //
    // forge script DeploySweepDepositERC4626Action

    // commands to deterministically deploy (and check the expected address before deploying):
    //
    // - with private key (on Anvil): forge script DeploySweepDepositERC4626Action --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeploySweepDepositERC4626Action --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeploySweepDepositERC4626Action --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeploySweepDepositERC4626Action --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        ...
        uint256 gasConstant = vm.envUint("SWEEP_DEPOSIT_ERC4626_ACTION_GAS_CONSTANT");
        ...
        // deterministically deploy SweepDepositERC4626Action via canonical Create2 deployer
        SweepDepositERC4626Action sweepDepositERC4626Action =
            new SweepDepositERC4626Action{salt: salt}(feeTokenRegistryAddress, treasuryAddress, gasConstant);

        vm.stopBroadcast();

        console2.log("SweepDepositERC4626Action deployed at:", address(sweepDepositERC4626Action));
    }
    ...
    function checkExpectedAddress(address feeTokenRegistry, address treasuryAddress, uint256 gasConstant) public view {
        ...
        address expectedAddress = vm.envAddress("EXPECTED_SWEEP_DEPOSIT_ERC4626_ACTION_ADDRESS");
        ...
        address calculatedAddress = vm.computeCreate2Address(
            ...
            keccak256(
                abi.encodePacked(
                    type(SweepDepositERC4626Action).creationCode,
