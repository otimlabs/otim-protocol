...
contract DeployTransferAction is Script {
    ...
    function run() public {
        ...
        // deterministically deploy TransferAction contracts via canonical Create2 deployer
        TransferAction transferAction =
            new TransferAction{salt: salt}(feeTokenRegistryAddress, treasuryAddress, gasConstant);

        vm.stopBroadcast();

        console2.log("TransferAction deployed at:", address(transferAction));
    }
    ...
    function checkExpectedAddress(address feeTokenRegistry, address treasuryAddress, uint256 gasConstant) public view {
        ...
        address calculatedAddress = vm.computeCreate2Address(
            ...
            keccak256(
                abi.encodePacked(
                    type(TransferAction).creationCode, abi.encode(feeTokenRegistry, treasuryAddress, gasConstant)
                )
            )
        );
