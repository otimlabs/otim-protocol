...
contract DeployCallOnceAction is Script {
    ...
    //
    // forge script DeployCallOnceAction

    // commands to deterministically deploy (and check the expected address before deploying):
    //
    // - with private key (on Anvil): forge script DeployCallOnceAction --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeployCallOnceAction --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeployCallOnceAction --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeployCallOnceAction --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        ...
        uint256 gasConstant = vm.envUint("CALL_ONCE_ACTION_GAS_CONSTANT");
        ...
        console2.log("CallOnceAction deployed at:", address(callOnceAction));
    }
    ...
    function checkExpectedAddress(
        ...
        address expectedAddress = vm.envAddress("EXPECTED_CALL_ONCE_ACTION_ADDRESS");
        ...
        address calculatedAddress = vm.computeCreate2Address(
            ...
            keccak256(
                abi.encodePacked(
                    type(CallOnceAction).creationCode,
