...
contract DeployWithdrawERC4626Action is Script {
    ...
    //
    // forge script DeployWithdrawERC4626Action

    // commands to deterministically deploy (and check the expected address before deploying):
    //
    // - with private key (on Anvil): forge script DeployWithdrawERC4626Action --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeployWithdrawERC4626Action --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeployWithdrawERC4626Action --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeployWithdrawERC4626Action --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        ...
        uint256 gasConstant = vm.envUint("WITHDRAW_ERC4626_ACTION_GAS_CONSTANT");
        ...
        WithdrawERC4626Action withdrawERC4626Action =
            new WithdrawERC4626Action{salt: salt}(feeTokenRegistryAddress, treasuryAddress, gasConstant);

        vm.stopBroadcast();

        console2.log("WithdrawERC4626Action deployed at:", address(withdrawERC4626Action));
    }
    ...
    function checkExpectedAddress(address feeTokenRegistry, address treasuryAddress, uint256 gasConstant) public view {
        ...
        address expectedAddress = vm.envAddress("EXPECTED_WITHDRAW_ERC4626_ACTION_ADDRESS");
        ...
        address calculatedAddress = vm.computeCreate2Address(
            ...
            keccak256(
                abi.encodePacked(
                    type(WithdrawERC4626Action).creationCode, abi.encode(feeTokenRegistry, treasuryAddress, gasConstant)
                )
            )
        );
