...
contract DeployTreasury is Script {
    ...
    //
    // forge script DeployTreasury

    // commands to deterministically deploy (and check the expected address before deploying):
    //
    // - with private key (on Anvil): forge script DeployTreasury --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeployTreasury --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeployTreasury --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeployTreasury --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        ...
        Treasury treasury = new Treasury{salt: salt}(owner);
    ...
    function checkExpectedAddress(address owner) public view {
        ...
        address expectedAddress = vm.envAddress("EXPECTED_TREASURY_ADDRESS");

        // calculate the expected address before deploying
        address calculatedAddress =
            vm.computeCreate2Address(salt, keccak256(abi.encodePacked(type(Treasury).creationCode, abi.encode(owner))));
