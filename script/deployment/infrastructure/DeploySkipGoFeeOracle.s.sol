...
contract DeploySkipGoFeeOracle is Script {
    ...
    //
    // forge script DeploySkipGoFeeOracle

    // commands to deterministically deploy (and check the expected address before deploying):
    //
    // - with private key (on Anvil): forge script DeploySkipGoFeeOracle --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeploySkipGoFeeOracle --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeploySkipGoFeeOracle --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeploySkipGoFeeOracle --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        address owner = vm.envAddress("SKIP_GO_FEE_ORACLE_OWNER_ADDRESS");
        ...
        console2.log("SkipGoFeeOracle deployed at:", address(skipGoFeeOracle));
    }
    ...
    function checkExpectedAddress(address owner) public view {
        ...
        address expectedAddress = vm.envAddress("EXPECTED_SKIP_GO_FEE_ORACLE_ADDRESS");

        // calculate the expected address before deploying
        address calculatedAddress = vm.computeCreate2Address(
            salt, keccak256(abi.encodePacked(type(SkipGoFeeOracle).creationCode, abi.encode(owner)))
        );
