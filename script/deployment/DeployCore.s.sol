...
contract DeployCore is Script {
    ...
    //
    // forge script DeployCore

    // commands to deterministically deploy (and check the expected address before deploying):
    //
    // - with private key (on Anvil): forge script DeployCore --broadcast --fork-url http://localhost:8545 --private-key $ANVIL_DEPLOYER_PK
    // - with private key:            forge script DeployCore --broadcast --rpc-url $RPC_URL --private-key $DEPLOYER_PK
    // - with Ledger:                 forge script DeployCore --broadcast --rpc-url $RPC_URL --ledger
    // - with AWS:                    forge script DeployCore --broadcast --rpc-url $RPC_URL --aws
    ...
    function run() public {
        ...
        // deterministically deploy core contracts via canonical Create2 deployer
        OtimDelegate otimDelegate = new OtimDelegate{salt: salt}(owner);

        vm.stopBroadcast();

        console2.log("OtimDelegate deployed at:", address(otimDelegate));
        console2.log("Gateway deployed at:", address(otimDelegate.gateway()));
        console2.log("InstructionStorage deployed at:", address(otimDelegate.instructionStorage()));
        console2.log("ActionManager deployed at:", address(otimDelegate.actionManager()));
    }
    ...
    function checkExpectedAddress(address owner) public view {
        ...
        address expectedAddress = vm.envAddress("EXPECTED_OTIM_DELEGATE_ADDRESS");

        // calculate the expected address using the current init code
        address calculatedAddress = vm.computeCreate2Address(
            salt, keccak256(abi.encodePacked(type(OtimDelegate).creationCode, abi.encode(owner)))
        );
