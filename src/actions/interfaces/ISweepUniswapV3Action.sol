...
bytes32 constant INSTRUCTION_TYPEHASH = keccak256(
    "Instruction(uint256 salt,uint256 maxExecutions,address action,SweepUniswapV3 sweepUniswapV3)Fee(address token,uint256 maxBaseFeePerGas,uint256 maxPriorityFeePerGas,uint256 executionFee)SweepUniswapV3(address recipient,address tokenIn,address tokenOut,uint24 feeTier,uint256 amountIn,uint256 floorAmountOut,uint32 meanPriceLookBack,uint32 maxPriceDeviationBPS,Fee fee)"
);

bytes32 constant ARGUMENTS_TYPEHASH = keccak256(
    "SweepUniswapV3(address recipient,address tokenIn,address tokenOut,uint24 feeTier,uint256 amountIn,uint256 floorAmountOut,uint32 meanPriceLookBack,uint32 maxPriceDeviationBPS,Fee fee)Fee(address token,uint256 maxBaseFeePerGas,uint256 maxPriorityFeePerGas,uint256 executionFee)"
);
