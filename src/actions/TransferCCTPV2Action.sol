...
/// @notice an Action that transfers ERC20 tokens from the user's account to a target on a different chain via CCTP V2
contract TransferCCTPV2Action is IAction, ITransferCCTPV2Action, Interval, OtimFee {
    ...
    function argumentsHash(bytes calldata arguments) public pure returns (bytes32, bytes32) {
        return (INSTRUCTION_TYPEHASH, hash(abi.decode(arguments, (TransferCCTPV2))));
    }

    /// @inheritdoc ITransferCCTPV2Action
    function hash(TransferCCTPV2 memory arguments) public pure returns (bytes32) {
        return keccak256(
            abi.encode(
                ARGUMENTS_TYPEHASH,
                arguments.token,
    ...
    function execute(
        ...
        TransferCCTPV2 memory arguments = abi.decode(instruction.arguments, (TransferCCTPV2));
